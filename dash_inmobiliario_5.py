...
def pyg_format(x: float) -> str:
    # Format with thousands separator as '.' (common in PY/AR Spanish locales)
...
with st.expander("Ingresar alquiler anual, años y tasa de descuento", expanded=False):
    ...
    with c2:
        anios_alquiler = st.number_input(
            ...
            value=1,
            step=1,
            help="Cantidad de años que se alquila el inmueble."
        )
    ...
    m1, m2 = st.columns(2)
    with m1:
        st.metric("Total nominal (PYG)", pyg_format(total_nominal_pyg))
    with m2:
        st.metric("Total descontado (valor presente) (PYG)", pyg_format(total_descontado_pyg))

    # (Opcional) detalle de flujos
    if n > 0 and alquiler_anual_pyg > 0:
        years = np.arange(1, n + 1)
        flujo = np.full(n, float(alquiler_anual_pyg))
        factor_desc = (1 + r) ** (-years) if r > 0 else np.ones(n)
